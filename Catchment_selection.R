...
# Basin data
q_metadata <- read.csv("Data/Zenodo/v11/Q_PMETobs_v11_metadata.csv", row.names = "gauge_id")
q_metadata$gauge_code <- as.character(q_metadata$gauge_code)
q_obs      <- read.csv("Data/Zenodo/v11/Q_PMETobs_1950_2020_v11d.csv", row.names = "Date")
q_obs      <- q_obs[rownames(q_obs) %in% as.character(period_calib), ] 
q_shape    <- vect("GIS South/Basins_PMETobs_points.shp")
...
# save
writeVector(q_shape, "GIS South/Basins_PMETobs_points_subset.shp", overwrite = TRUE)
